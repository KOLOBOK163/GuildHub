<template>
  <div class="management-container">
    <div class="header-actions">
      <button @click="showCreateModal = true" class="btn-primary">+ Добавить команду</button>
    </div>
      ...
      <table class="data-table">
        ...
        <tbody>
          <tr v-for="team in teams" :key="team.id">
            ...
            <td>
              <div class="team-info">
                <img v-if="team.logoUrl" :src="team.logoUrl" :alt="team.name" class="team-logo" />
                <span>{{ team.name }}</span>
              </div>
            </td>
            <td>{{ team.prizes || 0 }}</td>
            <td>{{ team.hltvRating || '-' }}</td>
            <td class="actions">
              <button @click="editTeam(team)" class="btn-edit">Редактировать</button>
              <button @click="deleteTeam(team.id)" class="btn-delete">Удалить</button>
            </td>
          </tr>
      ...
      <div class="modal-content" @click.stop>
        ...
        <form @submit.prevent="saveTeam">
          ...
          <div class="form-group">
            ...
            <input v-model="formData.logoUrl" type="url" />
          </div>
          <div class="form-group">
            <label>Призы</label>
          ...
          <div class="form-group">
            ...
            <input v-model.number="formData.hltvRating" type="number" step="0.01" min="0" max="10" />
          </div>
  ...
  methods: {
    ...
    editTeam(team) {
      ...
      this.formData = {
        ...
        prizes: team.prizes || 0,
        hltvRating: team.hltvRating || null
      };
    },
    async saveTeam() {
      try {
        ...
        const url = this.editingTeam
          ? `http://localhost:8080/api/teams/${this.editingTeam.id}`
